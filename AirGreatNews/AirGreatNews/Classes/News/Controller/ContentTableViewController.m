...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SXNewsEntity *NewsModel = self.arrayList[indexPath.row];
    if (NewsModel.hasHead && NewsModel.photosetID) {
        [self pushToDetailViewControllerWithUrl:NewsModel];
    }else if (NewsModel.hasHead){
        [self pushToDetailViewControllerWithUrl:NewsModel];
...
-(void)pushToDetailViewControllerWithUrl : (SXNewsEntity *) NewsModel{

    HomeDetailController *vc = [[HomeDetailController alloc] init];
    vc.topTitle = NewsModel.title;
    vc.time = NewsModel.ptime;
    vc.imageName = NewsModel.imgsrc;
    vc.desc = NewsModel.digest;
